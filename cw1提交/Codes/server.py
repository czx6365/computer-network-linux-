...
import math
import shutil
import struct  # Fix: Added missing struct module import

MAX_PACKET_SIZE = 20480

# Constants
OP_SAVE, OP_DELETE, OP_GET, OP_UPLOAD, OP_DOWNLOAD, OP_BYE, OP_LOGIN, OP_ERROR = 'SAVE', 'DELETE', 'GET', 'UPLOAD', 'DOWNLOAD', 'BYE', 'LOGIN', "ERROR"  # Operations
TYPE_FILE, TYPE_DATA, TYPE_AUTH, DIR_EARTH = 'FILE', 'DATA', 'AUTH', 'EARTH'  # File, Data, Authentication, Earth???
...
DIR_REQUEST, DIR_RESPONSE = 'REQUEST', 'RESPONSE'  # client--server  server--client
...
def get_file_md5(filename):
    ...
    """
    m = hashlib.md5()
    with open(filename, 'rb') as fid:  # Fix: Changed 'r' to 'rb' to read file in binary mode
        while True:
            d = fid.read(2048)
            if not d:
                break
...
def make_response_packet(operation, status_code, data_type, status_msg, json_data, bin_data=None):
    ...
    :return:
    """
    json_data[FIELD_OPERATION] = operation  # Operation type
    json_data[FIELD_DIRECTION] = DIR_RESPONSE  # Direction: response
    json_data[FIELD_STATUS] = status_code  # Status code
    json_data[FIELD_STATUS_MSG] = status_msg  # Status message
    json_data[FIELD_TYPE] = data_type
    return make_packet(json_data, bin_data)  # Final packet


def get_tcp_packet(conn):
    """
    Receive a complete TCP "packet" from a TCP stream and get the json data and binary data.
    :param conn: the TCP connection  TCP connection object (client socket)
    :return:
        json_data
        bin_data
    """
    bin_data = b''
    # read fixed 8-byte header: json length and binary length
    while len(bin_data) < 8:
        data_rec = conn.recv(8 - len(bin_data))
        if data_rec == b'':
            time.sleep(0.01)
        if data_rec == b'':
            return None, None
...
def data_process(username, request_operation, json_data, connection_socket):
    ...
    # 2. Process OP_SAVE (Save data)
    if request_operation == OP_SAVE:  # Fix: Changed op_save to OP_SAVE
        key = str(uuid.uuid4())
        ...
        try:
            with open(join('data', username, key), 'w') as fid:
                ...
                logger.info(f'<-- Data is saved with key "{key}"')  # Fix: Changed error to info
                connection_socket.send(
    ...
    if request_operation == OP_DELETE:
        ...
        try:
            ...
            logger.info(f'<-- The "key" {json_data[FIELD_KEY]} is deleted.')  # Fix: Changed error to info
            connection_socket.send(
...
def file_process(username, request_operation, json_data, bin_data, connection_socket):
    ...
    if request_operation == OP_SAVE:
        ...
        if FIELD_SIZE not in json_data.keys():
            ...
            connection_socket.send(
                make_response_packet(OP_SAVE, 410, TYPE_FILE, f'This file "size" has to be included', {}))  # Fix: Changed 402 to 410
            return
    ...
    if request_operation == OP_DELETE:
        if FIELD_KEY not in json_data.keys():
            ...
            connection_socket.send(
                make_response_packet(OP_DELETE, 410, TYPE_FILE, f'Field "key" is missing for FILE delete.', {}))  # Fix: Changed OP_GET to OP_DELETE
            return
        ...
        if os.path.exists(join('file', username, json_data[FIELD_KEY])) is False:
            if os.path.exists(join('tmp', username, json_data[FIELD_KEY])) is True:
                ...
                connection_socket.send(
                    make_response_packet(OP_DELETE, 404, TYPE_FILE,  # Fix: Changed OP_GET to OP_DELETE
                                         f'The "key" {json_data[FIELD_KEY]} is not completely uploaded. '
            ...
            logger.error(f'<-- The "key" {json_data[FIELD_KEY]} is not existing.')
            connection_socket.send(
                make_response_packet(OP_DELETE, 404, TYPE_FILE, f'The "key" {json_data[FIELD_KEY]} is not existing.', {}))  # Fix: Changed OP_GET to OP_DELETE
            return
        try:
            os.remove(join('file', username, json_data[FIELD_KEY]))
            logger.info(f'<-- The "key" {json_data[FIELD_KEY]} is deleted.')  # Fix: Changed error to info
            connection_socket.send(
                make_response_packet(OP_DELETE, 200, TYPE_FILE, f'The "key" {json_data[FIELD_KEY]} is deleted.',  # Fix: Changed OP_GET to OP_DELETE
                                     {FIELD_KEY: json_data[FIELD_KEY]}))
        except Exception as ex:
            logger.error(f'{str(ex)}@{ex.__traceback__.tb_lineno}')
    # upload
    if request_operation == OP_UPLOAD:
        ...
        if os.path.exists(join('file', username, json_data[FIELD_KEY])) is True:
            ...
            connection_socket.send(
                make_response_packet(OP_UPLOAD, 402, TYPE_FILE, f'The "key" {json_data[FIELD_KEY]} is completely uploaded.', {}))  # Fix: Changed 408 to 402
            return
    ...
    if request_operation == OP_DOWNLOAD:
        if FIELD_KEY not in json_data.keys():
            ...
            connection_socket.send(
                make_response_packet(OP_DOWNLOAD, 410, TYPE_FILE, f'Field "key" is missing for FILE downloading.', {}))  # Fix: Changed OP_GET to OP_DOWNLOAD
            return
        ...
        if os.path.exists(join('file', username, json_data[FIELD_KEY])) is False:
            if os.path.exists(join('tmp', username, json_data[FIELD_KEY])) is True:
                ...
                connection_socket.send(
                    make_response_packet(OP_DOWNLOAD, 404, TYPE_FILE,  # Fix: Changed OP_GET to OP_DOWNLOAD
                                         f'The "key" {json_data[FIELD_KEY]} is not completely uploaded. '
            ...
            logger.error(f'<-- The "key" {json_data[FIELD_KEY]} is not existing.')
            connection_socket.send(
                make_response_packet(OP_DOWNLOAD, 404, TYPE_FILE, f'The "key" {json_data[FIELD_KEY]} is not existing.', {}))  # Fix: Changed OP_GET to OP_DOWNLOAD
            return

        if FIELD_BLOCK_INDEX not in json_data.keys():
            logger.error(f'<-- The "block_index" is compulsory.')
            connection_socket.send(
                make_response_packet(OP_DOWNLOAD, 410, TYPE_FILE, f'The "block_index" is compulsory.', {}))  # Fix: Changed OP_GET to OP_DOWNLOAD
            return
        file_path = join('file', username, json_data[FIELD_KEY])
        file_size = getsize(file_path)
        block_size = MAX_PACKET_SIZE
        ...
        if block_index >= total_block:
            logger.error(f'<-- The "block_index" exceed the max index.')
            connection_socket.send(
                make_response_packet(OP_DOWNLOAD, 410, TYPE_FILE, f'The "block_index" exceed the max index.', {}))  # Fix: Changed OP_GET to OP_DOWNLOAD
            return
        if block_index < 0:
            logger.error(f'<-- The "block_index" should >= 0.')
            connection_socket.send(
                make_response_packet(OP_DOWNLOAD, 410, TYPE_FILE, f'The "block_index" should >= 0.', {}))  # Fix: Changed OP_GET to OP_DOWNLOAD
            return
...
def STEP_service(connection_socket, addr):
    ...
    while True:
        ...
        # Easter egg: Process "Three Body" "DO NOT ANSWER" test (DIR_EARTH)  ACK for "Three Body". If you never read the book "Three Body",
        # just understand the following part as an Echo function. This part is out of the protocol.
        # This is an Easter egg. Aha, this is a very good book.  66666666
...
def tcp_listener(server_ip, server_port):
    ...
    """
    global logger
    server_socket = socket(AF_INET, SOCK_STREAM)  # Fix: Changed SOCK_DGRAM to SOCK_STREAM, because this is a TCP server
    ...
    while True:
        try:
            ...
            th.daemon = True
            th.start()  # Fix: Added missing th.start() to start the thread
        except Exception as ex:
            logger.error(f'{str(ex)}@{ex.__traceback__.tb_lineno}')
...
def main():
    ...
    os.makedirs('tmp', exist_ok=True)  # Fix added: Create tmp directory

    tcp_listener(server_ip, server_port)  # Fix: Changed tcp-listener to tcp_listener
